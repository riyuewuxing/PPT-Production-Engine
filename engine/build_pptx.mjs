...
function resolveInsideRoot(relPath, label) {
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) throw new Error(`${label}: path escapes repository root: ${relPath}`);
  return resolved;
}
...
function pngSize(file) {
  ...
  if (b.length < 24 || b.subarray(0, 8).toString("hex") !== "89504e470d0a1a0a") throw new Error(`Only PNG previews are supported for aspect-safe placement: ${file}`);
...
function addSectionSlide(pptx, spec) {
  ...
  slide.addText(spec.body, { x: 0.75, y: 2.35, w: 9.6, h: 1.1, fontSize: 21, color: "3A4A57", fit: "shrink", margin: 0.05 });
}

function addBulletsSlide(pptx, spec) {
  const slide = pptx.addSlide(); slide.background = { color: "FAFBFC" };
  ...
  slide.addText(lines, { x: 1.0, y: 1.55, w: 9.7, h: 3.8, fontSize: 18, color: "273642", fit: "shrink", margin: 0.06, paraSpaceAfterPt: 10 });
}
...
function addTwoColumnSlide(pptx, spec) {
  ...
  const columns = [{ x: 0.8, title: spec.left_title, items: spec.left_items, color: "EAF3FF", accent: "2B6CB0" }, { x: 6.25, title: spec.right_title, items: spec.right_items, color: "EEFDF6", accent: "15803D" }];
  for (const col of columns) {
    slide.addShape(pptx.ShapeType.roundRect, { x: col.x, y: 1.45, w: 5.0, h: 4.6, fill: { color: col.color }, line: { color: "D9E2EC" } });
    slide.addText(col.title, { x: col.x + 0.35, y: 1.85, w: 4.25, h: 0.35, fontSize: 21, bold: true, color: col.accent, margin: 0.02 });
    slide.addText(col.items.map((b) => ({ text: b, options: { bullet: { type: "bullet" } } })), { x: col.x + 0.45, y: 2.55, w: 4.15, h: 2.7, fontSize: 16, color: "273642", fit: "shrink", margin: 0.05, paraSpaceAfterPt: 7 });
  }
}
...
function addTimelineSlide(pptx, spec) {
  ...
  const startX = 0.9, gap = 2.25, y = 3.05;
  for (let i = 0; i < spec.steps.length; i += 1) {
    const x = startX + i * gap;
    if (i < spec.steps.length - 1) slide.addShape(pptx.ShapeType.line, { x: x + 0.62, y: y + 0.25, w: gap - 0.2, h: 0, line: { color: "9AA6B2", width: 1.25, endArrowType: "triangle" } });
...
function addAssetSlide(pptx, spec) {
  ...
  const box = { x: 0.95, y: 1.48, w: 11.4, h: 4.5 };
  const fitted = containRect(imagePath, box);
  slide.addShape(pptx.ShapeType.roundRect, { x: 0.72, y: 1.25, w: 11.85, h: 4.95, fill: { color: "F8FAFC" }, line: { color: "D9E2EC", width: 1 } });
  slide.addImage({ path: imagePath, ...fitted });
  if (spec.caption) slide.addText(spec.caption, { x: 0.95, y: 6.35, w: 11.35, h: 0.45, fontSize: 11.5, color: "64748B", margin: 0.02, fit: "shrink" });
}
...
async function main() {
  ...
  const evidence = { status: "PASS", engine_version: manifest.engine_version, request_id: manifest.request_id, manifest: path.relative(ROOT, manifestPath).replaceAll("\\", "/"), pptx: path.relative(ROOT, outputPptx).replaceAll("\\", "/"), pptx_sha256: sha256(outputPptx), slide_count_declared: manifest.slides.length, generated_at: new Date().toISOString(), generator: "pptxgenjs" };
